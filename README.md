...
This project systematically studies cross-dataset generalization of Mask R-CNN between two publicly available underwater trash datasets: [**TrashCan 1.0**](https://conservancy.umn.edu/items/6dd6a960-c44a-4510-a679-efb8c82ebfb7) and [**SeaClear**](https://data.4tu.nl/datasets/4f1dff25-e157-4399-a5d4-478055461689). These datasets represent distinct underwater visual domains — deep-sea ROV footage versus shallow-water diver and ROV footage — with incompatible annotation schemes and substantially different imaging conditions. The central questions are: (1) how severe is the cross-dataset performance gap, (2) what drives it, and (3) can mitigation strategies like pooled training and category coarsening recover performance?
...
To address this, we implemented a **chunk-based splitting** procedure (`resplit_by_chunks.py`). Images are sorted lexicographically, grouped into consecutive chunks of 50 frames, and entire chunks are randomly assigned to train/val/test splits (75/15/10). This vastly reduces the probability that temporally adjacent frames land in both the training set and the val/test sets and consequently the severity of the data leakage from near-duplicate frames. Another obvious grouping strategy would be to assign all frames from a single video to one of the splits; however, this turns out to lead to very poorly stratified splits (where, for example, all or nearly all instances of certain classes appear in only one part of the split), especially in the case of SeaClear, which appears to comprise only a small number of videos with many frames each. Additionally, about half of the SeaClear images lack metadata indicating where one video ends and another begins, so splitting by video would require some guesswork as well. 
...
- **Synthetic Text Overlays**: Add synthetic text overlays to the SeaClear images similar to those already present in the TrashCan images; then, train a model on the resulting SeaClear+overlay images. The idea here was to teach the SeaClear model to ignore the text as it appeared the TrashCan model had already learned to do. This may not seem at first to be a domain-agnostic experiment because it was inspired by differences between two specific datasets, but the text overlays are essentially independent from everything else in the images and carry practically no information relevant to instance segmentation, so we consider it to be domain-agnostic. (See [`add_text_overlay.py`](https://github.com/crsprunger/underwater-segmentation-transfer/blob/main/tta/add_text_overlay.py) for the implementation of the data preprocessing step; this could also be done at test time, but since the overlays are baked into the TrashCan images, we wanted to mimic that.)
- **Random Grayscale**: Train another model on SeaClear and/or TrashCan, randomly (with probability 0.4) replacing each (RGB) image during training with the grayscale image of its luminance values. The idea here was to force the model not to rely so much on color-specific information. One could also, for example, randomly permute (and/or drop) input color channels, but the grayscale experiment was chosen for its simplicity.
...
- Domain-specific augmentation/adaptation methods (see section 7.5.2 above) may prove successful in increasing cross-dataset performance.
