...
- **TC20** (20 classes): A contiguous re-indexing of TrashCan's categories excluding two classes (`animal_crab`, `animal_eel`) that have no SeaClear equivalent. SeaClear's 40 categories map into this space via a manually constructed mapping.
- **Coarse** (5 classes): Groups TC20 categories by extraction difficulty — `rov`, `organic` (plants, animals, branches), `trash_easy` (small extractable items like bottles and cans), `trash_entangled` (flexible items with snagging risk, like rope and nets), and `trash_heavy` (large/heavy items requiring human review).
...
Training uses **SGD** with momentum 0.9, learning rate 0.005, weight decay 1e-4, cosine annealing with linear warmup (1 epoch), and mixed-precision (AMP). Multi-scale training samples from image sizes (256, 288, 320, 352, 384) with max dimension 704. **Copy-paste augmentation** is enabled for the primary models, pasting 1–3 randomly scaled and rotated object instances onto training images. Training runs for 50 epochs, and the checkpoint with the best validation mask mAP is selected.
